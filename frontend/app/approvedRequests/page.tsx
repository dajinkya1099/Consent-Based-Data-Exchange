  ...
  const filteredRequests = useMemo(
    () =>
      sampleRequests.filter((request) => {
        ...
        return (
          request.consumerName.toLowerCase().includes(query) ||
          request.resource.toLowerCase().includes(query) ||
          request.constraints.toLowerCase().includes(query) ||
          request.expiryAt.toLowerCase().includes(query) ||
      ...
      <div className="mx-auto max-w-[1440px] space-y-6">
        <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-semibold text-slate-900">Approved Requests</h1>
              
            </div>

            <div className="flex w-full max-w-md items-center gap-3 rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3">
              <label htmlFor="approved-search" className="sr-only">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-6 py-4 font-semibold text-slate-600">Consumer Name</th>
                  <th className="px-6 py-4 font-semibold text-slate-600">Resource</th>
                  <th className="px-6 py-4 font-semibold text-slate-600">Constraints</th>
                  <th className="px-6 py-4 font-semibold text-slate-600">ExpiryAt</th>
              ...
              <tbody className="divide-y divide-slate-200 bg-white">
                {filteredRequests.length > 0 ? (
                  filteredRequests.map((request) => (
                    <tr key={request.id}>
                      <td className="px-6 py-4 text-slate-900">{request.consumerName}</td>
                      <td className="px-6 py-4 text-slate-900">{request.resource}</td>
                      <td className="px-6 py-4 text-slate-700">{request.constraints}</td>
                      <td className="px-6 py-4 text-slate-900">{request.expiryAt}</td>
                      <td className="px-6 py-4 text-slate-900">{request.status}</td>
